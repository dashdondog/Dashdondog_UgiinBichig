...
interface BodyProps {
  ...
  formatYear: (d?: string) => number | null;
}

function MemberDetailBody({ member, parents, spouses, children, formatYear }: BodyProps) {
  return (
    <>
      {/* Photo & name */}
...
export default function MemberDetail({ member, onClose }: Props) {
  ...
  return (
    <>
      {/* ── Mobile: bottom sheet ── */}
      <div className="sm:hidden fixed inset-0 z-40 bg-black/20" onClick={onClose} />
      <div className="sm:hidden fixed bottom-0 left-0 right-0 z-50 bg-white rounded-t-2xl shadow-2xl flex flex-col max-h-[75vh]">
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-4">
          <MemberDetailBody member={member} parents={parents} spouses={spouses} children={children} formatYear={formatYear} />
        </div>
      </div>

      {/* ── Desktop: side panel ── */}
      <div className="hidden sm:flex h-full w-80 bg-white border-l border-green-100 shadow-xl flex-col flex-shrink-0">
